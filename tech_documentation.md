...
### Core Architecture & Logic
* **Instrumentation-aware patch modeling (`gap_type='patch'`):** Mimics structural or viewing constraints of a real particle detector by assuming a fixed set of look directions for the instrument. The degrader combines those fixed look directions with magnetic field directions and identifies the pitch-angle bins where the geometry produces a near-zero dot product $\mu = \hat{u}_p \cdot \hat{B}$. Those bins are then treated as blind spots and zeroed out across the spatial domain.
* **Geometry-driven random masking (`gap_type='random'`):** Extends the same physics-based logic to a probabilistic masking mode. Instead of masking arbitrary points, the routine chooses from the pitch-angle bins that are geometrically consistent with the particle-direction and magnetic-field-direction combination, so the dropout pattern follows the instrument geometry.
* **Time-series support:** The module accepts 3D inputs with shape `(time, space, mu)` and can apply a different blind spot at each time step when the magnetic field direction changes. It also exposes a helper for rendering a pitch-angle-versus-time color map instead of a single line plot.
